...
IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG = @ApplicationDatabaseName AND TABLE_NAME = 'Cutomers'))
BEGIN
...
IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_CATALOG = @ApplicationDatabaseName AND TABLE_NAME = 'CutomersHistories'))
BEGIN
	CREATE TABLE CutomersHistories (
		Id INT PRIMARY KEY IDENTITY,
		CustomerId INT FOREIGN KEY REFERENCES Cutomers(Id) NOT NULL,
		BenefitsHistoryId INT FOREIGN KEY REFERENCES BenefitsHistories(Id) NOT NULL,
		Firstname NVARCHAR(255) NOT NULL,
		Lastname NVARCHAR(255) NOT NULL,
		BasicSalary INT NOT NULL,
		Birthdate DATETIME NOT NULL,
